...
class Learner(ABC):
	...
	def train_epoch(self, dataset, lr=0.0001, epoch=None, return_metric=False):
		...
		for batch_idx, (mic_sig_batch, gt_batch) in pbar:
			...
			with torch.cuda.amp.autocast(enabled=self.use_amp):
				pred_batch = self.model(in_batch) # [1, 24, 512]
				# loss_batch = self.mse_loss(pred_batch = pred_batch, gt_batch = gt_batch)
			
				loss_batch = self.ce_loss(pred_batch = pred_batch, gt_batch = gt_batch)
			...
			if return_metric: 
				...
				metric_batch = self.evaluate_cls(pred=pred_batch, gt=gt_batch)
				if batch_idx==0:
	...
	def test_epoch(self, dataset, return_metric=False):
       ...
       # mic_sig_batch[1, 76640, 2] 
       # gt_batch_doa [1, 24, 2, 1] gt_batch_vad_sources[1, 24, 3328, 1]
				print('-----------------------')
				in_batch, gt_batch = self.data_preprocess(mic_sig_batch, gt_batch)
	...
	def predict_batch(self, gt_batch, mic_sig_batch, wDNN=True):
		...
		with torch.no_grad():
			...
			# mic_sig_batch = mic_sig_batch.to(self.device) # [1, 312197, 0]
			in_batch, gt_batch = self.data_preprocess(mic_sig_batch, gt_batch)
	...
	def predict_uncertainty(self, dataset, wDNN=True, return_predgt=False, metric_setting=None,save_file=False):
		""" 
			Function: Predict 
			Args:
		...
		with torch.no_grad():
			...
			for mic_sig_batch, gt_batch in dataset:
				...
				mic_sig_batch = torch.cat((mic_sig_batch[:,:,8:9], mic_sig_batch[:,:,5:6]), axis=-1) # [bs, 312197, 0]
				in_batch, gt_batch = self.data_preprocess(mic_sig_batch, gt_batch)
				print(in_batch.shape)
				# obtain the evidence
				pred_batch = self.model(in_batch) # [bs, t, 180]
				nb, nt, _ = pred_batch.shape
				pred_doa_softmax = F.softmax(pred_batch.reshape(nb*nt,-1),dim=-1)    
				pred_doa_evi = pred_batch.reshape(nb*nt,-1)
				evidence = torch.exp(pred_doa_evi)
				alpha = evidence + 1
				...
				# print(U)
				# print(U.shape)
				pred_scores, pred_cls = torch.max(pred_doa_softmax, dim=-1)
				evidence_scores, evidence_cls = torch.max(evidence, dim=-1)
				alpha_scores, alpha_cls = torch.max(alpha, dim=-1)				
				# obtain the predicted doa
				...
				with open("/fn-ssl/locata_data/locata_uncertainty/pred_scores.txt", "a+") as f:
					f.write(str(pred_scores.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/pred_cls.txt", "a+") as f:
					f.write(str(pred_cls.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/evidence_scores.txt", "a+") as f:
					f.write(str(evidence_scores.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/evidence_cls.txt", "a+") as f:
					f.write(str(evidence_cls.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/alpha_scores.txt", "a+") as f:
					f.write(str(alpha_scores.detach().cpu().numpy()) + "\n")	
				with open("/fn-ssl/locata_data/locata_uncertainty/evidence.txt", "a+") as f:
					f.write(str(evidence.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/alpha.txt", "a+") as f:
					f.write(str(alpha.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/pred_doa.txt", "a+") as f:
					f.write(str(DOA_batch_pred.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/pred_doa_softmax.txt", "a+") as f:
					f.write(str(pred_doa_softmax.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/S.txt", "a+") as f:
					f.write(str(S.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/U.txt", "a+") as f:
						for item in U:
							f.write("%s\n" % item)

				if (metric_setting is not None):
					if save_file:
						metric_batch = self.evaluate_cls(pred=pred_batch, gt=gt_batch,metric_setting=metric_setting, idx=idx)
					else:
						metric_batch = self.evaluate_cls(pred=pred_batch, gt=gt_batch,metric_setting=metric_setting)
				if return_predgt:
					pred += [pred_batch]
					gt += [gt_batch]
	...
	def predict(self, dataset, wDNN=True, return_predgt=False, metric_setting=None,save_file=False):
		...
		with torch.no_grad():
			...
			for mic_sig_batch, gt_batch in dataset:
				...
				# uncertainy calculation
				pred_doa = pred_batch["doa"]
				nb, nt, _, _ = pred_doa.shape
				pred_doa_softmax = F.softmax(pred_doa.reshape(nb*nt,-1),dim=-1)
    
				pred_doa = pred_doa.reshape(nb*nt,-1)
				evidence = torch.exp(torch.clamp(pred_doa, min=-10, max=10))
				alpha = evidence + 1
				S = torch.sum(alpha,dim=1,keepdim=True)
				U = 180 / S	
				pred_scores, pred_cls = torch.max(pred_doa_softmax, dim=-1)
				...
				with open("/fn-ssl/locata_data/locata_uncertainty/pred_scores.txt", "w") as f:
					f.write(str(pred_scores.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/pred_cls.txt", "w") as f:
					f.write(str(pred_cls.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/evidence_scores.txt", "w") as f:
					f.write(str(evidence_scores.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/evidence_cls.txt", "w") as f:
					f.write(str(evidence_cls.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/alpha_scores.txt", "w") as f:
					f.write(str(alpha_scores.detach().cpu().numpy()) + "\n")	
				with open("/fn-ssl/locata_data/locata_uncertainty/evidence.txt", "w") as f:
					f.write(str(evidence.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/alpha.txt", "w") as f:
					f.write(str(alpha.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/pred_doa.txt", "w") as f:
					f.write(str(pred_doa.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/pred_doa_softmax.txt", "w") as f:
					f.write(str(pred_doa_softmax.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/S.txt", "w") as f:
					f.write(str(S.detach().cpu().numpy()) + "\n")
				with open("/fn-ssl/locata_data/locata_uncertainty/U.txt", "w") as f:
					f.write(str(U.detach().cpu().numpy()) + "\n")
				if (metric_setting is not None):
					if save_file:
						metric_batch = self.evaluate_cls(pred=pred_batch, gt=gt_batch,metric_setting=metric_setting, idx=idx)
					else:
...
class SourceTrackingFromSTFTLearner(Learner):
	...
	def __init__(self, model, win_len, win_shift_ratio, nfft, fre_used_ratio, nele, nazi, rn, fs, ch_mode, tar_useVAD, localize_mode, c=343.0): #, arrayType='planar', cat_maxCoor=False, apply_vad=False):
		...
		self.nele = nele # maps resolution
		self.nazi = nazi
	...
	def data_preprocess(self, mic_sig_batch=None, gt_batch=None, vad_batch=None, eps=1e-6, nor_flag=True):
		...
		if mic_sig_batch is not None:
			...
			stft = self.dostft(signal=mic_sig_batch) # (nb,nf,nt,nch) locata: [1, 257, 1221, 0]
			stft = stft.permute(0, 3, 1, 2)  # (nb,nch,nf,nt) [1, 2, 257, 298] locata: [1, 0, 257, 1221]

			# change batch (nb,nch,nf,nt)→(nb*(nch-1),2,nf,nt)/(nb*(nch-1)*nch/2,2,nf,nt)
			stft_rebatch = self.addbatch(stft) # [1, 2, 257, 298]
			if nor_flag:
				nb, nc, nf, nt = stft_rebatch.shape
				mag = torch.abs(stft_rebatch) # abs 
				mean_value = forgetting_norm(mag) # [bs, 1, 1, 298]
				stft_rebatch_real = torch.real(stft_rebatch) / (mean_value + eps)
				stft_rebatch_image = torch.imag(stft_rebatch) / (mean_value + eps)				
			else:
				stft_rebatch_real = torch.real(stft_rebatch) # [1, 2, 257, 298]
				stft_rebatch_image = torch.imag(stft_rebatch) # [1, 2, 257, 298]
			# prepare model input
			real_image_batch  =  torch.cat((stft_rebatch_real,stft_rebatch_image),dim=1) # real_image_batch [1, 4, 257, 298]
			data += [real_image_batch[:,:,self.fre_range_used,:]] # [1, 4, 256, 298] when the frequency is equal to 0 which will be ommitted.

		if gt_batch is not None:
			DOAw_batch = gt_batch['doa'] # [1, 24, 2, 1]
			vad_batch = gt_batch['vad_sources'] # [1, 24, 3328 ,1]
			...
			ipd_batch = torch.from_numpy(ipd_batch) # [1, 24, 512, 1, 1]

			vad_batch = vad_batch.mean(axis=2).float() # (nb,nseg,nsource) # s>2/3 

			# DOAw_batch = torch.from_numpy(source_doa).to(self.device) 
			DOAw_batch = DOAw_batch.to(mic_sig_batch.device) # (nb,nseg,2,nsource) [1, 24, 2, 1]
			ipd_batch = ipd_batch.to(mic_sig_batch.device) # [1, 24, 512, 1, 1]
			vad_batch = vad_batch.to(mic_sig_batch.device) # [1, 24, 1]

			if self.tar_useVAD:
				nb, nt, nf, nmic, num_source = ipd_batch.shape
				th = 0
				vad_batch_copy = deepcopy(vad_batch) # [2, 24, 1]
				vad_batch_copy[vad_batch_copy<=th] = th
				vad_batch_copy[vad_batch_copy>0] = 1
				vad_batch_expand = vad_batch_copy[:, :, np.newaxis, np.newaxis, :].expand(nb, nt, nf, nmic, num_source) # [1, 24, 512, 1, 1]
				ipd_batch = ipd_batch * vad_batch_expand
			ipd_batch = torch.sum(ipd_batch, dim=-1)   # (nb,nseg,2nf,nmic-1) [1, 24, 512, 1]
	...
	def ce_loss(self, pred_batch=None, gt_batch=None):
		...
		pred_doa_softmax = F.softmax(pred_doa.reshape(nb*nt,-1),dim=-1)
		# evidence = F.relu(pred_doa.reshape(nb*nt,-1))	
		pred_doa = pred_doa.reshape(nb*nt,-1)
		evidence = torch.exp(torch.clamp(pred_doa, min=-10, max=10))
		alpha = evidence + 1
		S = torch.sum(alpha,dim=1,keepdim=True)
		...
		with open("/fn-ssl/MONITOR/pred_scores.txt", "w") as f:
			f.write(str(pred_scores.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/pred_cls.txt", "w") as f:
			f.write(str(pred_cls.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/evidence_scores.txt", "w") as f:
			f.write(str(evidence_scores.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/evidence_cls.txt", "w") as f:
			f.write(str(evidence_cls.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/alpha_scores.txt", "w") as f:
			f.write(str(alpha_scores.detach().cpu().numpy()) + "\n")	
		with open("/fn-ssl/MONITOR/gt_doa.txt", "w") as f:
			f.write(str(gt_doa.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/evidence.txt", "w") as f:
			f.write(str(evidence.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/alpha.txt", "w") as f:
			f.write(str(alpha.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/pred_doa.txt", "w") as f:
			f.write(str(pred_doa.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/pred_doa_softmax.txt", "w") as f:
			f.write(str(pred_doa_softmax.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/S.txt", "w") as f:
			f.write(str(S.detach().cpu().numpy()) + "\n")
		with open("/fn-ssl/MONITOR/U.txt", "w") as f:
			f.write(str(U.detach().cpu().numpy()) + "\n")
		label = F.one_hot(gt_doa.reshape(nb*nt), num_classes=180) # [48, 180]
		loss = torch.sum(label * (torch.digamma(S) - torch.digamma(alpha)), dim=1, keepdim=True)
	...
	def mse_loss(self, pred_batch=None, gt_batch=None):
		...
		Returns:
			loss
        """
		pred_ipd = pred_batch # [1, 24, 512]
		gt_ipd = gt_batch['ipd'] # [1, 24, 512, 1]
		nb, _, _, _ = gt_ipd.shape # (nb, nt, nf, nmic)

		pred_ipd_rebatch = self.removebatch(pred_ipd, nb).permute(0, 2, 3, 1) # [1, 24, 512, 1]

		loss = torch.nn.functional.mse_loss(pred_ipd_rebatch.contiguous(), gt_ipd.contiguous().to(pred_ipd_rebatch.device))
	...
	def predgt2DOA(self, pred_batch=None, gt_batch=None, time_pool_size=None):
		...
		if pred_batch is not None:
			...
			dpipd_template = torch.from_numpy(dpipd_template).to(pred_batch.device) # (nele, nazi, 2nf, nmic)

			# !!!
			nele, nazi, _, _ = dpipd_template.shape
			dpipd_template = dpipd_template[int((nele-1)/2):int((nele-1)/2)+1, int((nazi-1)/2):nazi, :, :] # [1, 37, 512, 1]
	...
	def evaluate(self, pred, gt, metric_setting={'ae_mode':['azi'], 'ae_TH':5, 'useVAD':True, 'vad_TH':[2/3, 2/3], 'metric_unfold':False},idx=None ):
		...
		Returns:
			metric
        """
		doa_gt = (gt['doa'] * 180 / np.pi).to(pred['doa'].device)   
		doa_pred = pred['doa'] * 180 / np.pi 
		vad_gt = gt['vad_sources'].to(pred['doa'].device)   
		vad_pred = pred['vad_sources'] 
		if idx != None:
			save_path = './locata_result/'
			np.save(save_path+str(idx)+'_gt',doa_gt.cpu().numpy())
