...
import webrtcvad
import gpuRIR
from utils.simu import load_file
...
# THE SETTINGS FOR THE MICROPHONE ARRAY
...
class AcousticScene:
	...
	def simulate(self):
		...
		if self.T60 == 0:
			...
			Tdiff = gpuRIR.att2t_SabineEstimator(12, self.T60) # Use ISM until the RIRs decay 12dB  # Time to start the diffuse reverberation model [s]
			Tmax = gpuRIR.att2t_SabineEstimator(40, self.T60)  # Use diffuse model until the RIRs decay 40dB # Time to stop the simulation [s]
			if self.T60 < 0.15: Tdiff = Tmax # Avoid issues with too short RIRs
			nb_img = gpuRIR.t2n( Tdiff, self.room_sz ) # Number of image sources in each dimension
		...
		noise_signal = np.sqrt(ac_pow/10**(self.SNR/10)) / np.sqrt(ac_pow_noise) * self.noise_signal # 同比例放大
		mic_signals += noise_signal[0:len(self.t), :] # shape: [76640, 2]
		...
		if hasattr(self, 'source_vad'):
			...
			for source_idx in range(num_source):
				...
				vad_sources = vad[0:len(self.t),:].mean(axis=1) > vad[0:len(self.t),:].max()*1e-3 # threshold for voice activity is 0.001
...
class NoiseDataset():
	...
	def get_random_noise(self, mic_pos=None):
		...
		if noise_type == 'spatial_white':
			...
			# the length of noise_copy is the same as the nsample_desired
	...
	def _exploreCorpus(self, path, file_extension):
		...
		# directory_tree: 'machinegun':'/fn-ssl/data/NoiSig/Noise92/machinegun.wav'
		# directory_path: 0:'/fn-ssl/data/NoiSig/Noise92/machinegun.wav'
...
class LocataDataset(Dataset):
	...
	def __init__(self, paths, array, fs, tasks=(1,3,5), recording=None, dev=False, transforms = None, return_acoustic_scene=False):
		...
		self.vad.set_mode(1) # soft vad mode
		
		if array == 'dicit':
			self.array_setup = dicit_array_setup
		if array == 'dummy':
			self.array_setup = dualch_array_setup
		self.directories = []
		for path in paths:
			for task in tasks:
	...
	def __getitem__(self, idx):
		...
		if self.dev:
			...
			DOA_pts = np.zeros(sources_pos.shape[0:2] + (2,)) # [1, 2797, 2]
			DOA = np.zeros(trajectories.shape[0:2] + (2,)) # [1, 317980,]
			for s in range(sources_pos.shape[0]):
				source_pos_local = np.matmul( np.expand_dims(sources_pos[s,...] - array_pos, axis=1), array_rotation ).squeeze() # [2797, 3]
    			# np.matmul( array_rotation, np.expand_dims(sources_pos[s,...] - array_pos, axis=-1) ).squeeze()
...
class RandomTrajectoryDataset(Dataset):
	...
	def __init__(self, sourceDataset, num_source, source_state, room_sz, T60, abs_weights, array_setup, array_pos, noiseDataset, SNR, nb_points, min_dis, c=343.0, transforms=None):
		...
		"""
		self.sourceDataset = sourceDataset # signals from libspeech
		self.source_state = source_state # mobile
	...
	def getRandomScene(self, idx):
		...
		beta = gpuRIR.beta_SabineEstimation(room_sz, T60, abs_weights) # Reflection Coefficients
  ...
  # the timestamp for each sampling point
    ...
    # np.newaxis add a more axis

				if np.random.random(1) < 0.25:
					traj_pts[:,:,source_idx] = np.ones((self.nb_points,1)) * src_pos_ini
			#set the ele = pi/2
			traj_pts[:,2,:] = mic_pos[0,2] # shape: [50, 3, 1]
			# Interpolate trajectory points
   			# traj.shape[76640, 3, 1]
			trajectory[:,:,source_idx]  = np.array([np.interp(t, timestamps, traj_pts[:,i,source_idx]) for i in range(3)]).transpose()
			# cartestian coordinate transform to spherical coordiante
			DOA[:,:,source_idx] = cart2sph(trajectory[:,:,source_idx] - array_pos)[:, 1:3] # [r, phi, theta][:,1:3]->[phi, theta] [76640, 2, 1]
		acoustic_scene = AcousticScene(
			room_sz = room_sz,
			T60 = T60,
			beta = beta,
